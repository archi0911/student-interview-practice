    ...
    <div className="page max-w-5xl mx-auto pt-8">
      <div className="animate-fadeInUp">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
          <div>
            <div className="inline-block px-3 py-1 rounded-full border border-orange-500/30 bg-orange-500/10 text-orange-400 text-xs font-medium mb-3">
              Admin Portal
            </div>
            <h1 className="text-3xl font-bold">Admin Dashboard</h1>
            <p className="text-[var(--text-muted)] text-sm mt-1">Platform overview and user management</p>
          </div>
        </div>
                  ...
                  filteredUsers.map(user => (
                    <tr key={user.id} className="border-b border-[var(--border)]/50 hover:bg-black/10 transition-colors">
                      ...
                      <td className="p-4 text-right">
                        <Link to={`/admin/users/${user.id}`} className="text-[var(--accent-light)] hover:text-white text-sm font-medium transition-colors">
                          View details →
                        </Link>
                      </td>
                    </tr>
